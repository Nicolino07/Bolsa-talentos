...
CREATE TABLE persona (
    ...
    telefono VARCHAR(30),
    disponible VARCHAR(20) CHECK (disponible IN ('MAÑANA', 'TARDE', 'NOCHE', 'FULL')),
...
CREATE TABLE coincidencia (
    ...
    dni INTEGER NOT NULL REFERENCES persona(dni) ON DELETE CASCADE,
    id_oferta INTEGER NOT NULL REFERENCES oferta_empleo(id_oferta) ON DELETE CASCADE,
    puntaje_total DECIMAL(5,4) NOT NULL CHECK (puntaje_total BETWEEN 0 AND 1),
    detalles JSONB,
    fecha_calculo TIMESTAMP DEFAULT NOW(),
    notificado BOOLEAN DEFAULT FALSE,
    UNIQUE(dni, id_oferta)
);
...
CREATE INDEX idx_coincidencia_dni ON coincidencia(dni);
CREATE INDEX idx_coincidencia_oferta ON coincidencia(id_oferta);
